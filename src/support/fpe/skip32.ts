/* eslint-disable @typescript-eslint/no-non-null-assertion */
/* 
  Ported from: https://github.com/femto113/node-skip32/blob/master/skip32.js
  The original comment block follows.
  ---
  Skip32.js - public domain javascript implementation of:

  SKIP32 -- 32 bit block cipher based on SKIPJACK.
  Written by Greg Rose, QUALCOMM Australia, 1999/04/27.

  In common: F-table, G-permutation, key schedule.
  Different: 24 round feistel structure.
  Based on:  Unoptimized test implementation of SKIPJACK algorithm 
             Panu Rissanen <bande@lut.fi>

  SKIPJACK and KEA Algorithm Specifications 
  Version 2.0 
  29 May 1998

  Not copyright, no rights reserved.
*/

const _buf = Buffer.from([
  ,
]);

const ftable = new Uint8Array(_buf);

export class Skip32 {
  // NOTE: we expose a KEYLEN constant in case anything needs to code against it,
  // but we don't reference it in this module for performance reasons
  static KEYLEN = 10;

  private key: Uint8Array;

  constructor(key: string | Array<number>, lpadZero?: boolean) {
    let parsedKey: Array<number>;
    // If key is a hex string parse it into an array of bytes
    if (typeof key === 'string') {
      parsedKey = key
        .split(/(..)/)
        .filter((s) => s)
        .map((s) => parseInt(s, 16));
    } else {
      parsedKey = key;
    }

    // if lpadZero parameter given and truthy left pad array with 0 as needed
    if (typeof lpadZero !== 'undefined' && lpadZero) {
      while (parsedKey.length < 10) {
        parsedKey.unshift(0);
      }
    }

    this.key = new Uint8Array(10);
    for (let i = 0; i < 10; i++) {
      this.key[i] = parsedKey[i % parsedKey.length]!;
    }
  }

  private init() {}

  private round16(k: number, n: number) {
    let g1 = (n >> 8) & 0xff;
    let g2 = (n >> 0) & 0xff;
    let g3 = ftable[g2 ^ this.key[(4 * k + 0) % 10]!]! ^ g1;
    let g4 = ftable[g3 ^ this.key[(4 * k + 1) % 10]!]! ^ g2;
    let g5 = ftable[g4 ^ this.key[(4 * k + 2) % 10]!]! ^ g3;
    let g6 = ftable[g5 ^ this.key[(4 * k + 3) % 10]!]! ^ g4;
    return (g5 << 8) + g6;
  }

  private core(n: number, k: number, d: number) {
    let i;
    let wl = (((n >> 24) & 0xff) << 8) + (((n >> 16) & 0xff) << 0);
    let wr = (((n >> 8) & 0xff) << 8) + (((n >> 0) & 0xff) << 0);
    for (i = 0; i < 24 / 2; i++) {
      wr ^= this.round16(k, wl) ^ k;
      k += d;
      wl ^= this.round16(k, wr) ^ k;
      k += d;
    }
    return ((wr << 16) | wl) >>> 0;
  }

  encrypt(n: number) {
    return this.core(n, 0, 1);
  }

  decrypt(n: number) {
    return this.core(n, 23, -1);
  }
}
